...
class LocalRecentlyViewedDataSourceImp(
    ...
    override suspend fun getAllRecentlyViewed(): List<MediaDto> {
        ...
        return recentItems.mapNotNull { recentItem ->
            ...
            recentItems.indexOfFirst { item ->
                when(val dto = it) {
                    is MovieDto -> item.mediaId == dto.id && item.mediaType.equals("movie", true)
                    is TvShowDto -> item.mediaId == dto.id && item.mediaType.equals("tvshow", true)
                    else -> false
                }
            }
        }
