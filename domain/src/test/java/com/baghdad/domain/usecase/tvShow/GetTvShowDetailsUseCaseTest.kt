...
class GetTvShowDetailsUseCaseTest {
    ...
    @Test
    fun `should return cast members when repository returns data`() = runTest {
        val tvId = 100L
        coEvery { tvShowRepository.getTvShowCastMembers(tvId) } returns castMembers

        val result = getTvShowCastMembersUseCase(tvId)

        assertEquals(castMembers, result)
    }

    @Test
    fun `should return empty list when no cast members exist`() = runTest {
        val tvId = 101L
    ...
    @Test
    fun `should handle special characters in actor names`() = runTest {
        val tvId = 103L
    ...
    @Test
    fun `should return cast members with minimal required fields`() = runTest {
        val tvId = 104L
    ...
    @Test
    fun `should preserve all actor details in returned cast`() = runTest {
        val tvId = 105L
        coEvery { tvShowRepository.getTvShowCastMembers(tvId) } returns castMembers
    ...
    @Test
    fun `should make exactly one repository call per invocation`() = runTest {
        val tvId = 106L
        coEvery { tvShowRepository.getTvShowCastMembers(tvId) } returns castMembers
